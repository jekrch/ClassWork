...
namespace BattleShip.BLL.GameLogic
{
    public class Board
    {
        ...
        public string[,] BoardImage = new string[11, 11];
        ...
        // creates board image that displays hits and misses
        public void CreateBoardImg()
        {
            BoardImage[0, 0] = "     ";

            for (int x = 1; x < 11; x++)
            {
                // Display lettered column headings
                BoardImage[x, 0] = $"  {(char) (x + 64)}  "; 

                for (int y = 1; y < 11; y++)
                {
                    // open position markers
                    BoardImage[0, y] = $" {y} ".PadRight(5, ' '); 
                    BoardImage[x, y] = " | | ";
        ...
        // Update board to show players ship placements.
        public void UpdateBoardImg()
        {
                foreach (var ship in _ships)
                {
                    if (ship != null)
                    {   
                        // foreach placed ship, fill its coords on board image with ':' instead of '|'
                        foreach (var coord in ship.BoardPositions)
                        {
                            BoardImage[coord.XCoordinate, coord.YCoordinate] =
                                BoardImage[coord.XCoordinate, coord.YCoordinate].Replace("|", ":");
                        }
                    }
                }
        }
